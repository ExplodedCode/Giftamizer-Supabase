...
#   - bash (Git Bash on Windows works; this is not ported to PowerShell -
#     it shells out to Docker directly and needs root/sudo, which has no
...
preflight() {
    ...
    [ -f docker-compose.yml ] || die "Run this script from the repo root (where docker-compose.yml lives)."
